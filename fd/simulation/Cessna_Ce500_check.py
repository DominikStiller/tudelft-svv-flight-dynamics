...
dt = 0.2
t = np.arange(0, 1000*dt+dt, dt)
x_0 = np.array([V0, 0, 0, 0])
yout, t = ml.step(sys, t, x_0)
fig, axs = plt.subplots(2, 2, sharex=True)

axs[0, 0].plot(t, yout[:, 0])
axs[0, 0].set_title("Absolut deviation in airspeed")

axs[1, 0].plot(t, yout[:, 1])
axs[1, 0].set_title("Angle of attack")

axs[0, 1].plot(t, yout[:, 2])
axs[0, 1].set_title("Pitch angle")

axs[1, 1].plot(t, yout[:, 3])
